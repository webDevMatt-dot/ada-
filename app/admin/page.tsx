...
export default function AdminDashboard() {
    ...
    const moduleGroups: { title: string; items: DashboardModule[] }[] = [
        {
            title: "Overview",
            items: [
                {
    ...
    const visibleGroups = moduleGroups.map(group => ({
        ...group,
        items: group.items.filter(m => !m.adminOnly || (currentUser?.is_superuser || currentUser?.is_staff))
            ...
            <div className="space-y-10">
                {visibleGroups.map((group, idx) => (
                    <div key={idx} className="space-y-4">
                        {group.title && (
                            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider pl-1">{group.title}</h3>
                        )}
