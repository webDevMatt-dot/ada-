...
import { Plus, Loader2, Calendar as CalendarIcon, Users, Check, X, RotateCcw, Power, Trash2, Eye, Search } from "lucide-react";
...
export default function UpdatesAdminPage() {
        ...
        if (currentUser && updates.length > 0) {
            ...
            if (newDenied.length > 0) {
                if (ignoreNextPopup.current) {
                    ...
                    newDenied.forEach(u => {
                        if (!ignored.includes(u.id)) ignored.push(u.id);
                    });
    ...
    const handleAction = async (id: number, action: string, payload: any = {}) => {
        ...
        try {
            const res = await fetch(`/api/updates/${id}/${action}`, { // No trailing slash
                method: "POST",
                headers: {
            ...
            if (res.ok) {
                await fetchUpdates(); // Wait for fetch
    ...
    const handleBulkDelete = async () => {
        if (!confirm(`Are you sure you want to delete ${selectedIds.size} updates?`)) return;

        setActionLoading(true);
        // Sequential delete for simplicity (avoiding complexity of new backend bulk endpoint for now)
        // If > 50 items, we should consider a bulk endpoint.
        const ids = Array.from(selectedIds);

        for (const id of ids) {
            await handleAction(id, 'delete_soft');
        }
    ...
    const filteredUpdates = (status: string) => {
        return updates.filter(u => u.status === status);
    };

    const UpdateCard = ({ update }: { update: Update }) => {
        const isOwner = currentUser?.id === update.created_by;
        const isAdmin = currentUser?.is_superuser || currentUser?.is_staff;
        const canDelete = isAdmin || (isOwner && (update.status === 'pending' || update.status === 'review'));
        const isSelected = selectedIds.has(update.id);

        return (
            <Card className={`overflow-hidden flex flex-col group hover:shadow-md transition-all ${isSelected ? 'ring-2 ring-[#8b1d2c]' : ''}`}>
                <div className="aspect-video w-full bg-slate-100 relative overflow-hidden group/image">
                    {update.image ? (
                        <img
                            src={update.image}
                            alt={update.title}
                    ...
                    {/* Checkbox Overlay */}
                    {canDelete && update.status !== 'deleted' && (
                        <div className="absolute top-3 left-3 z-10">
                            <input
                                ...
                                className="w-5 h-5 rounded border-slate-300 text-[#8b1d2c] focus:ring-[#8b1d2c]"
                            />
                        </div>
                    )}
                ...
                <CardFooter className="pt-0 border-t border-slate-50 mt-auto p-4 bg-slate-50/50 flex flex-wrap gap-2">
                    {update.status !== 'deleted' && (
                        <div className="flex w-full gap-2">
                            ...
                            {(isOwner && (update.status === 'pending' || update.status === 'review')) && (
                                <Button
                                    size="sm"
                                    variant="outline"
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex gap-2 w-full sm:w-auto">
                    {selectedIds.size > 0 && (
                        ...
                        >
                            {actionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4 mr-2" />}
                            Delete Selected ({selectedIds.size})
                        </Button>
                    )}
                    <Link href="/admin/updates/new">
                        <Button className="bg-[#8b1d2c] hover:bg-[#6d1722] gap-2 w-full sm:w-auto">
            ...
            <div className="relative">
                <input
                    ...
                    className="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#8b1d2c]/20 focus:border-[#8b1d2c]"
                />
                ...
                <div className="space-y-6">
                    ...
                    {updates.filter(u => {
                        ...
                        <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50">
                            <p className="text-slate-500">No updates found matching "{searchQuery}"</p>
                        </div>
                    ) : (
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsList className="bg-slate-100 p-1 rounded-xl mb-6 flex-wrap h-auto">
                        {['pending', 'live', 'review', 'inactive', 'deleted'].map(status => (
                            <TabsTrigger key={status} value={status} className="rounded-lg data-[state=active]:bg-white data-[state=active]:text-[#8b1d2c] data-[state=active]:shadow-sm capitalize">
                                {status === 'review' ? 'Review' : status} ({filteredUpdates(status).length})
                            </TabsTrigger>
                        ))}
                    </TabsList>
